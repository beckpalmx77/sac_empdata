...
<script>
    function GET_DATA(table_name, idx) {
        ...
        let action = "GET_SUM_RESULT_COND"; // Default action

        let cond = "";
        let field = "leave_day";
        let is_leave_sum = false; // Flag สำหรับการรวมวันลา+ชั่วโมง
        ...
        switch (idx) {
            case 1:
                ...
                break;
            case 2:
                // 🎯 ใช้ GET_SUM_LEAVE_COND สำหรับลากิจ
                action = "GET_SUM_LEAVE_COND";
                is_leave_sum = true;
                table_name = "dleave_event";
                cond = " Where leave_type_id = 'L1' AND status = 'A' " + where_emp_id + where_year;
                break;
            case 3:
                // 🎯 ใช้ GET_SUM_LEAVE_COND สำหรับลาพักผ่อน
                action = "GET_SUM_LEAVE_COND";
                is_leave_sum = true;
                table_name = "dleave_event";
                ...
                break;
            case 4:
                // 🎯 ใช้ GET_SUM_LEAVE_COND สำหรับลาป่วย
                action = "GET_SUM_LEAVE_COND";
                is_leave_sum = true;
                table_name = "dleave_event";
        ...
        $.ajax({
            ...
            success: function (response) {
                if (is_leave_sum) {
                    // 🎯 สำหรับ ลากิจ, ลาพักผ่อน, ลาป่วย (idx 2, 3, 4)
                    try {
                        let data = JSON.parse(response);
                        let days = data.days || 0;
                        let hours = data.hours || 0;
                        input_text.innerHTML = "ใช้ไปแล้วจำนวน " + days + " วัน " + hours + " ชั่วโมง";
                    } catch (e) {
                        console.error("Failed to parse JSON response for leave sum:", response);
                        input_text.innerHTML = "ไม่พบข้อมูล";
                    }
                } else {
                    // 🎯 สำหรับวันหยุดอื่นๆ (idx 1, 5, 6, 7, 8)
